...
>
	<div class="bp-header">
		<Code size={12} />
		<span class="bp-title">{data.label || 'Code'}</span>
		{#if showComplete}
	...
	<div class="bp-body">
		...
		{#if data.description}
			<div class="bp-prop"><span class="bp-key">desc</span><span class="bp-val">{data.description}</span></div>
		{/if}
		{#if data.code}
			<div class="bp-prop"><span class="bp-key">code</span><span class="bp-val">{String(data.code).split('\n')[0].slice(0, 30)}{String(data.code).length > 30 ? '...' : ''}</span></div>
		{/if}
	</div>
...
<style>
	.bp-node {
		...
		border-radius: 8px;
		overflow: hidden;
		box-shadow: 0 4px 16px rgba(59, 130, 246, 0.12), 0 0 0 1px rgba(59, 130, 246, 0.2);
	...
	.bp-node.exec-running::after {
		...
		border-radius: 10px;
		padding: 2px;
		background: conic-gradient(from 0deg, transparent 0%, #3b82f6 30%, transparent 60%);
	...
	.bp-header {
		...
		padding: 0 10px;
		height: 32px;
		background: linear-gradient(135deg, #3b82f6 0%, color-mix(in srgb, #3b82f6 70%, #000) 100%);
		color: white;
		font-size: 11px;
		font-weight: 700;
	...
	.bp-pin-in, .bp-pin-out { display: flex; align-items: center; gap: 4px; }
	.bp-pin-in { font-size: 9px; color: var(--color-text-muted); }
	.bp-pin-out { font-size: 9px; color: var(--color-text-muted); }
	.bp-dot { width: 6px; height: 6px; border-radius: 50%; background: #3b82f6; flex-shrink: 0; }
