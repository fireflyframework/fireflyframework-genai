...
"""FireflyAgent -- the primary agent abstraction in the Firefly Agentic framework.
...
registration in the :class:`~fireflyframework_agentic.agents.registry.AgentRegistry`,
lifecycle hooks, and sensible defaults drawn from
:class:`~fireflyframework_agentic.config.FireflyAgenticConfig`.
"""
...
class FireflyAgent(Generic[AgentDepsT, OutputT]):
    ...
    * **Convention defaults** -- the default model and retry count are pulled
      from :func:`~fireflyframework_agentic.config.get_config` when not
      explicitly provided.
    * **Decorator proxies** -- :meth:`tool`, :meth:`tool_plain`, and
    ...
    async def run(
        ...
        *prompt* can be a plain string, a sequence of multimodal
        :class:`~fireflyframework_agentic.types.UserContent` parts (text,
    ...
    def _resolve_tools(tools: Sequence[Any]) -> list[Any]:
        ...
        * :class:`~fireflyframework_agentic.tools.base.BaseTool` instances are
          wrapped in :class:`pydantic_ai.Tool` so their ``execute`` method is
          called by the underlying agent.
        * :class:`~fireflyframework_agentic.tools.toolkit.ToolKit` instances are
