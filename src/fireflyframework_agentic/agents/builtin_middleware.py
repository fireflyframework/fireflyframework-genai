...
class LoggingMiddleware:
    ...
    **entry** log line in ``before_run`` and a **completion** log line in
    ...
    ``fireflyframework_agentic`` hierarchy so that
    :func:`~fireflyframework_agentic.logging.configure_logging` controls
...
class PromptGuardMiddleware:
    ...
    *sanitise* is ``True``, the prompt is sanitised (suspicious fragments
    ...
    Parameters:
        guard: A :class:`~fireflyframework_agentic.security.prompt_guard.PromptGuard`
...
class ExplainabilityMiddleware:
    ...
    :class:`~fireflyframework_agentic.explainability.trace_recorder.TraceRecorder`.
...
class CacheMiddleware:
    ...
    Parameters:
        cache: A :class:`~fireflyframework_agentic.agents.cache.ResultCache`.
    """
...
class OutputGuardMiddleware:
    ...
    Parameters:
        guard: An :class:`~fireflyframework_agentic.security.output_guard.OutputGuard`
...
class ValidationMiddleware:
    ...
    :class:`~fireflyframework_agentic.exceptions.OutputReviewError` is raised.
