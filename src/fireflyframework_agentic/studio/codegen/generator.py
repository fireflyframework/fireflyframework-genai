...
The generated code uses the fireflyframework-agentic public API so that users
...
def _emit_imports(type_set: set[NodeType], has_edges: bool) -> list[str]:
    ...
    if NodeType.AGENT in type_set:
        imports.append("from fireflyframework_agentic.agents.base import FireflyAgent")

    if NodeType.TOOL in type_set:
        imports.append("from fireflyframework_agentic.tools.registry import tool_registry")

    if NodeType.REASONING in type_set:
        imports.append("from fireflyframework_agentic.agents.registry import agent_registry")
        imports.append("from fireflyframework_agentic.reasoning.registry import reasoning_registry")

    if has_edges:
        imports.append("from fireflyframework_agentic.pipeline.builder import PipelineBuilder")
        ...
        if step_types:
            imports.append(f"from fireflyframework_agentic.pipeline.steps import {', '.join(sorted(step_types))}")

        imports.append("from fireflyframework_agentic.pipeline.context import PipelineContext")

    if NodeType.MEMORY in type_set:
        imports.append("from fireflyframework_agentic.memory.manager import MemoryManager")
        imports.append("from fireflyframework_agentic.memory.store import FileStore")
...
# ---------------------------------------------------------------------------
# Node emitters — one function per NodeType
# ---------------------------------------------------------------------------
