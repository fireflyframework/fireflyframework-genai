...
async def _lifespan(app: Any) -> AsyncIterator[None]:
    ...
    from fireflyframework_agentic.studio.settings import apply_settings_to_env, load_settings
    ...
    from fireflyframework_agentic.studio.custom_tools import CustomToolManager
...
def create_studio_app(
    ...
    Parameters:
        config: Optional :class:`~fireflyframework_agentic.studio.config.StudioConfig`.
    ...
    pkg_version = importlib.metadata.version("fireflyframework-agentic")
    ...
    # -- Settings endpoints ------------------------------------------------
    from fireflyframework_agentic.studio.api.settings import create_settings_router

    app.include_router(create_settings_router(settings_path))

    # -- Registry endpoints ------------------------------------------------
    from fireflyframework_agentic.studio.api.registry import create_registry_router

    app.include_router(create_registry_router())

    # -- Project endpoints -------------------------------------------------
    from fireflyframework_agentic.studio.api.projects import create_projects_router
    ...
    from fireflyframework_agentic.studio.api.project_api import create_project_api_router

    app.include_router(create_project_api_router(project_manager))

    # -- Version history endpoints -------------------------------------------
    from fireflyframework_agentic.studio.api.projects import create_versioning_router

    app.include_router(create_versioning_router(project_manager))

    # -- Custom tools endpoints --------------------------------------------
    from fireflyframework_agentic.studio.api.custom_tools import create_custom_tools_router
    from fireflyframework_agentic.studio.custom_tools import CustomToolManager
    ...
    from fireflyframework_agentic.studio.api.files import create_files_router

    app.include_router(create_files_router(project_manager))

    # -- Evaluation endpoints ----------------------------------------------
    from fireflyframework_agentic.studio.api.evaluate import create_evaluate_router

    app.include_router(create_evaluate_router(project_manager))

    # -- Experiments endpoints ---------------------------------------------
    from fireflyframework_agentic.studio.api.experiments import create_experiments_router

    app.include_router(create_experiments_router(project_manager))

    # -- Code generation endpoints -----------------------------------------
    from fireflyframework_agentic.studio.api.codegen import create_codegen_router

    app.include_router(create_codegen_router())

    # -- Monitoring endpoints ----------------------------------------------
    from fireflyframework_agentic.studio.api.monitoring import create_monitoring_router

    app.include_router(create_monitoring_router())

    # -- Checkpoint endpoints ----------------------------------------------
    from fireflyframework_agentic.studio.api.checkpoints import create_checkpoints_router
    ...
    # -- Execution WebSocket -----------------------------------------------
    from fireflyframework_agentic.studio.api.execution import create_execution_router

    app.include_router(create_execution_router())

    # -- Assistant WebSocket -----------------------------------------------
    from fireflyframework_agentic.studio.api.assistant import create_assistant_router

    app.include_router(create_assistant_router())

    # -- Oracle WebSocket & REST -------------------------------------------
    from fireflyframework_agentic.studio.api.oracle import create_oracle_router

    app.include_router(create_oracle_router())

    # -- Smith WebSocket (code generation) ---------------------------------
    from fireflyframework_agentic.studio.api.smith import create_smith_router

    app.include_router(create_smith_router())

    # -- GraphQL endpoint --------------------------------------------------
    from fireflyframework_agentic.studio.api.graphql_api import create_graphql_router

    app.include_router(create_graphql_router(project_manager))

    # -- Tunnel management -------------------------------------------------
    from fireflyframework_agentic.studio.api.tunnel import create_tunnel_router
...
def _get_default_static_dir() -> Path:
    ...
    if getattr(sys, "frozen", False):
        return Path(sys._MEIPASS) / "fireflyframework_agentic" / "studio" / "static"  # type: ignore[attr-defined]
